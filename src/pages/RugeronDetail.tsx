...
const RugeronDetail = () => {
  ...
  if (!rugeron) {
    return (
      <>
        <Helmet>
          <title>Rugerón No Encontrado</title>
          <meta name="description" content="El rugerón solicitado no fue encontrado." />
        </Helmet>
  ...
  const currentUrl = `${window.location.origin}/rugeron/${rugeron.id}`;
  const thumbnailUrl = `${window.location.origin}${rugeron.thumbnail}`;
  const moviesList = rugeron.movies.map(m => `${m.title} (${m.year})`).join(', ');

  return (
    <>
      <Helmet>
        <title>{rugeron.title} - Rugerón</title>
        <meta name="description" content={`Rugerón: ${rugeron.title} - ${formattedDate} en ${rugeron.place}. Películas: ${moviesList}. Asistentes: ${rugeron.attendants}`} />
        
        {/* Open Graph / Facebook */}
        <meta property="og:type" content="article" />
        <meta property="og:url" content={currentUrl} />
        <meta property="og:title" content={`${rugeron.title} - Rugerón`} />
        <meta property="og:description" content={`Rugerón: ${rugeron.title} - ${formattedDate} en ${rugeron.place}. Películas: ${moviesList}. Asistentes: ${rugeron.attendants}`} />
        ...
        <meta name="twitter:title" content={`${rugeron.title} - Rugerón`} />
        <meta name="twitter:description" content={`Rugerón: ${rugeron.title} - ${formattedDate} en ${rugeron.place}. Películas: ${moviesList}. Asistentes: ${rugeron.attendants}`} />
